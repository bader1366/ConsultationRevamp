...
**مخطط توضيحي نصي (RTL؛ تنويه الاشتباه الثابت أعلاه يعلو البطاقة دائماً):**
...
│ 2026-06-20 مخالفة صحيحة (بصمة v2) · 2026-07-02 stale: v3      │
└──────────────────────────────────────────────────────────────┘
```
...
**آلة الحالات (لكل `violation_finding` داخل الحالة؛ محدثة للقرارات الست):**

```mermaid
stateDiagram-v2
    [*] --> pending : كشف + تحقق R7 (violation_finding)
    pending --> in_review : إسناد من قائد المراجعين [ASSUME OD-30]
    in_review --> approved : مخالفة صحيحة + رمز سبب + مبرر + بصمة
    in_review --> rejected : ليست مخالفة + رمز سبب + مبرر
    in_review --> in_review : إعادة تصنيف إلى نوع آخر (حدث إلحاقي، فئة جديدة)
    in_review --> needs_second_review : تحتاج مراجعة ثانية
    needs_second_review --> in_review : مراجع ثانٍ مستقل؛ الخلاف إلى المحكّم
    in_review --> insufficient_evidence : أدلة غير كافية
    insufficient_evidence --> in_review : دليل جديد / إعادة فتح
    in_review --> pending_policy : إحالة لمالك السياسة
    pending_policy --> in_review : إيضاح التعريف (قد يرفع إصدار التصنيف)
...
**صلاحيات حسب الدور [ASSUME OD-11 + OD-30]:** reviewer: القرارات الست على حالاته المسندة (رفضه نافذ، قابل للنقض الإلحاقي من admin)؛ قائد المراجعين/المحكّم: الإسناد + حسم مجموعة الخلاف + فرض مراجعة ثانية؛ steward: قراءة فقط؛ admin: كل ما سبق + `retire`؛ service_owner لا يرى إلا مؤشرات معتمدة **مجمعة** بلا أسماء — لا يُستخدم هذا المسار للمراقبة المفتوحة دون صلاحية وسبب [DECISION owner 2026-08-03, Amendment §12-16].

**الحالات:** فارغ: «قائمة اشتباه المخالفة خالية. آخر تشغيل كشف: {timestamp}.» · جزئي: الحالات المعاد فتحها لتغير النص/الكاشف تُعزل في تبويب «أُعيد فتحها (stale)» بعدّاد بارز، ولا تُخلط بالجديدة. · خطأ: قياسية؛ فشل حفظ القرار لا يفقده — يُحفظ محلياً في الجلسة ويُعاد الإرسال.

**قواعد الصياغة:** قبل الاعتماد: «مؤشر مشتبه به» حصراً؛ بعده: «حالة معتمدة ضمن {VIOL-xxx}»؛ لا تظهر كلمة «إدانة» أو «مخالف» كوصف شخص في أي نص واجهة [DECISION §2.4]؛ والتنويه الثابت (أول هذا القسم) لا يُحذف من أي عرض لقائمة الاشتباه أو حالاتها.

**الوصول وRTL:** أزرار القرارات الست بألوان + نص + أيقونات متمايزة، وترتيبها ثابت لا يتبدل بين الحالات؛ اختصارات لوحة مفاتيح للمراجعة السريعة (مع تأكيد دائماً)؛ القوائم المغلقة (الفئة، رمز السبب) قابلة للتنقل بلوحة المفاتيح.

**من يراها:** §2.6 (SCR-08) — أشد الشاشات تقييداً بعد كشف PII.

**معايير قبول الشاشة** [DECISION owner 2026-08-03, Amendment §5.7 — تُنقل حرفياً إلى حزمة اختبارات الوثيقة 15]:

1. كل اقتباس يطابق الترانسكربت النشط حرفياً (بوابة R7)؛
2. لا يمكن اعتماد حالة دون مراجع مصرح له (تفويض خادمي — I14)؛
3. لا تظهر حالة معتمدة ضمن «مشتبه» بعد تحديث الصفحة؛
4. تعديل الترانسكربت أو إعادة الاستخراج يبطل القرار القديم أو يعيد الحالة للمراجعة وفق البصمة؛
5. يمكن قبول مؤشر ورفض آخر في الجلسة نفسها؛
6. كل انتقال ظاهر في سجل التدقيق وغير قابل للحذف؛
7. لا تدخل أي حالة غير معتمدة في رقم قيادي على أنها مخالفة مؤكدة — حجم قائمة الاشتباه يُعرض رقماً منفصلاً معلَّماً.
...
## 15. الشاشات التشغيلية الجديدة SCR-14…SCR-22 (تعديل المالك 2026-08-03)

تسع شاشات جديدة قررها تعديل المالك لتخدم الحلقات التشغيلية الست والرحلات ر13–ر16؛ أسماؤها ومعرفاتها أدناه ثابتة، وSCR-01…SCR-13 كما هي (مع ترقية SCR-08 في §10) [DECISION owner 2026-08-03, Amendment §12-18]. كلها تلتزم الأسس المشتركة في §2 حرفياً: الحالات الأربع (§2.5)، مصفوفة الأدوار (§2.6 — صفوفها مضافة)، المسارات (§2.7)، أحداث التدقيق (§2.9)، وقاموس الصياغة (§2.4). عقود الخدمة في الوثيقة 17؛ نموذج البيانات في الوثيقة 08؛ خطوات التشغيل في الوثيقة 09.

**أعلام الميزات الحاكمة** (العلم عزلُ تعرضٍ لا بديل عن اكتمال الميزة) [DECISION owner 2026-08-03, Amendment §10.5]: `nightly_consolidation_enabled` → SCR-14 وSCR-21 · `session_360_enabled` → SCR-15 وSCR-16 · `violation_review_enabled` → SCR-08 وSCR-17 (وتتغذى منهما SCR-18) · `action_center_enabled` → SCR-19 · `monthly_infographic_enabled` → SCR-20.

**قاعدة عرض مشتركة إضافية:** كل الأرقام في هذه الشاشات من بيانات فعلية عبر الخدمة — **ممنوع بنيوياً أي قيمة تجريبية Hardcoded في أي لوحة** [DECISION owner 2026-08-03, Amendment §10.4-7]، وتُفصل دائماً أعداد «الاشتباه» عن «المعتمد» في أي عدّاد.

### 15.1 SCR-14 — مركز تشغيل البيانات (CAP-OPS-02 · ر13 · VS-03)

**الغرض:** الشاشة الواحدة التي تجيب: ماذا جرى في التشغيل الليلي الموحد؟ ما نجح وما فشل ومتى؟ — بلا نبش في السجلات [DECISION owner 2026-08-03, Amendment §4.5].

**العناصر:**

1. **بطاقة آخر تشغيل:** `run_id`، الحالة من الحالات السبع الحرفية `queued → running → partial → succeeded → failed → cancelled → superseded`، وقتا البداية والنهاية، نسخ الكود والعقود والنماذج والبرومبتات والتصنيفات، وعدد محاولات إعادة التشغيل [DECISION owner 2026-08-03, Amendment §4.3].
2. **حالة كل مصدر:** مزود التفريغ + كل عقد فرعي من عائلة SRC-DATAHUB على حدة: آخر نجاح، علامة المياه (Watermark)، السجلات المسحوبة/الجديدة/المعدلة/المرفوضة — فشل عقد واحد يجعل التشغيل `partial` ظاهراً ولا يمنع بقية العقود [DECISION owner 2026-08-03, Amendment §3.3-7].
3. **عدد الجلسات الجديدة والمعدلة** في آخر تشغيل، والمكتملة والمستبعدة والمتأخرة.
4. **مراحل الـPipeline الست عشرة** (الوثيقة 09) بمدة كل مرحلة وحالتها؛ العناصر الفاشلة تدخل DLQ بسبب محدد — لا تحويل فشل إلى نجاح صامت (I16).
5. **الجلسات غير المطابقة** (رابط إلى طوابير SCR-11).
6. **الجلسات بلا ترانسكربت.**
7. **عدد حالات الاشتباه الجديدة** (رابط SCR-08 — عدّاد «مشتبه» منفصل عن أي عدّاد معتمد).
8. **عدد حالات المراجعة المتأخرة عن SLA** [ASSUME OD-30].
9. **اكتمال الشهر الحالي** مقابل بوابة ≥98% [DECISION owner 2026-08-03, Amendment §13].
10. **الإجراءات:** إعادة محاولة العناصر الفاشلة (`rerun failed items`)، إعادة معالجة جلسات محددة (`reprocess selected sessions`)، فتح مشكلة، مشاهدة السجلات، تنزيل Run manifest [DECISION owner 2026-08-03, Amendment §4.4-8].

**الحالات:** فارغ: «لم يُنفَّذ أي تشغيل بعد. التشغيل الليلي يبدأ عند 02:00 بتوقيت الرياض [ASSUME OD-28].» · جزئي: التشغيل `partial` حالة أولى الدرجة: تُعرض الخطوات الناجحة والفاشلة بأسبابها، ولا يُعرض «نجاح» مجمل · خطأ: قياسية §2.5 لكل لوحة على حدة.

**من يراها:** §2.6 (SCR-14). **التدقيق:** `PIPELINE_RETRY_REQUESTED` / `SESSION_REPROCESS_REQUESTED` (§2.9).

### 15.2 SCR-15 — الجلسة 360 (CAP-OPS-03 · VS-01)

**الغرض:** المكان الواحد الذي تُرى فيه الجلسة كاملة من المصدرين: النص المفرّغ + بيانات DataHub + نواتج الإثراء + منشأ كل حقل وتغطيته [DECISION owner 2026-08-03, Amendment §2.1 وPB-004].

**العناصر:**

1. **رأس الهوية:** `advisory_session` ULID، التاريخ والمدة، البرنامج/النافذة/القناة/فئة الخدمة، المستشار (وفق الصلاحية)، حالة الجلسة من DataHub، وشارة مصدر الحقيقة لكل حقل (مصفوفة Field Authority — الوثيقة 05): «الحالة والموعد: DataHub يحكم؛ النص: مزود التفريغ النشط».
2. **تبويب النص:** عارض اللفات الكامل (SCR-03 مضمّناً) بإصدار المصدر النشط — النص الخام لا يُمس (I6).
3. **تبويب البيانات الداخلية:** تقييم المستفيد وملاحظاته، تقييم المستشار وملاحظاته وتوصياته، نتيجة الجلسة واحتياج المتابعة، الحضور والإلغاء — كل قيمة بختم `source_record_version` و`observed_at`، والمتأخرة تُعلَّم «وصلت في تشغيل {تاريخ}» (دعم البيانات المتأخرة) [DECISION owner 2026-08-03, Amendment §3.3-5].
4. **تبويب التحليل:** النتائج المستخرجة (خطوات واضحة، تحديات، مؤشرات رضا وضغط…) بأختامها الكاملة (I13)، وأي اشتباه مخالفة بحالة مراجعته وبالتنويه المعتاد — غير المعتمد يُعرض «مشتبهاً» حصراً.
5. **زر «إضافة اشتباه لم يرصده النظام»** → يفتح SCR-17 [DECISION owner 2026-08-03, Amendment §6.3].
6. **كتلة التغطية:** الحقول الغائبة وسببها (تأخر مصدر، عدم مطابقة، جلسة بلا تفريغ) — الغياب معلن لا مطموس (I8).

**الحالات:** فارغ: لا ينطبق (تُفتح من SCR-16 أو من روابط) · جزئي: جلسة بلا ترانسكربت تعرض البيانات الداخلية مع سبب الغياب — صفحة صالحة لا خطأ؛ تقييم لم يصل بعد يُعرض «لم يصل من المصدر حتى {آخر تشغيل}» · خطأ: قياسية.

**من يراها:** §2.6 (SCR-15). **التدقيق:** `SESSION_360_VIEWED`؛ وفتح النص الكامل يسجل `EVIDENCE_ACCESS` كالمعتاد.

### 15.3 SCR-16 — بحث الجلسات (CAP-OPS-04 · VS-01)

**الغرض:** الوصول السريع إلى الجلسات بفلاتر محكومة — مدخل «الجلسة 360» [DECISION owner 2026-08-03, Amendment PB-005].

**العناصر:** فلاتر مغلقة من السجل: الفترة، البرنامج/النافذة، المستشار (ضمن الصلاحية)، حالة الجلسة، وجود ترانسكربت نشط، تقييم المستفيد (نطاق)، حالة المطابقة بين المصدرين؛ جدول نتائج بترقيم خادمي؛ كل صف يفتح SCR-15. لا بحث نصياً حراً في هذه الشاشة — البحث الدلالي في الأدلة شأن قدرات لاحقة (موجة C، بعد مشاورة SD-21).

**الحالات:** فارغ: «لا جلسات ضمن هذه الفلاتر.» مع كتلة النطاق · جزئي: فلتر على حقل مصدره متأخر يعلن ذلك في كتلة التغطية · خطأ: قياسية.

**من يراها:** §2.6 (SCR-16) — نفس نمط SCR-15. **التدقيق:** البحث بفلاتر مغلقة لا يسجَّل؛ فتح الجلسة يسجَّل (`SESSION_360_VIEWED`).

### 15.4 SCR-17 — الإبلاغ عن اشتباه فائت (CAP-OPS-06 · ر14 · VS-02/VS-04)

**الغرض:** التقاط ما فاته الكاشف — بدون هذه الميزة يقيس النظام ما اكتشفه فقط ولا يعرف ما فاته؛ بلاغاتها مصدر مجموعة False negatives في حلقة التعلم المحكومة (SD-22) وتقدير الاسترجاع (EXP-11) [DECISION owner 2026-08-03, Amendment §6.3].

**العناصر (حوار يُفتح من SCR-15 أو عارض النص الكامل بزر «إضافة اشتباه لم يرصده النظام»):**

1. **تحديد المقطع:** اختيار لفة أو مدى لفات؛ الاقتباس يُلتقط حرفياً من النص النشط (بوابة R7 تسري عليه كأي اقتباس).
2. **اختيار النوع:** إحدى الفئات `VIOL-001…008` بأسمائها الحرفية، أو **«نوع جديد»** بوصف حر — يتحول تلقائياً مقترحَ فئة في طابور الحوكمة (ر9/SCR-09).
3. **سبب الإبلاغ:** نص إلزامي (≥15 حرفاً).
4. **الإرسال:** ينشئ `missed_violation_report` ويفتح حالة في SCR-08 معلَّمة «بلاغ بشري»، تذهب وجوباً **لمراجعة ثانية مستقلة عن المبلِّغ** [DECISION owner 2026-08-03, Amendment §6.3].
5. **قائمة «بلاغاتي»:** حالة كل بلاغ (قيد المراجعة/حُسم) دون كشف قرارات الآخرين.

**الحالات:** فارغ: «لا بلاغات سابقة. يُنشأ البلاغ من شاشة الجلسة 360.» · جزئي: لا ينطبق · خطأ: قياسية؛ فشل الإرسال يحفظ المسودة في الجلسة ويعاد الإرسال.

**من يراها:** §2.6 (SCR-17). **التدقيق:** `MISSED_VIOLATION_REPORTED`.

### 15.5 SCR-18 — لوحة جودة الكاشف والتعلم (CAP-OPS-07 · VS-04)

**الغرض:** الوجه المرئي لحلقة تحسين الكاشف المحكومة: قرارات المراجعين تتحول بياناتٍ معنونة بإصدارات، ولا نقرة مراجع تغير سلوك الإنتاج مباشرة (SD-22) [DECISION owner 2026-08-03, Amendment §6].

**العناصر:**

1. **مقاييس الجودة لكل نوع `VIOL-xxx`** [DECISION owner 2026-08-03, Amendment §6.6]: الدقة، الاسترجاع المقدَّر (من العينة السلبية الأسبوعية [ASSUME OD-32] وتصميم EXP-11)، معدل قبول المراجعين `detector_acceptance_rate`، معدل إعادة التصنيف، معدل خلاف المراجعين، متوسط زمن المراجعة، نسبة المتأخر عن SLA، نسبة الحالات المبطلة بتغير الأساس، والانحراف حسب الشهر/البرنامج/جودة التفريغ — كل النسب **بمجالات ثقة، وتُحجب أي نسبة على n<30** مع عرض العدد.
2. **مجموعات البيانات المعنونة:** إصدار كل Dataset (ثابت لا يعدل)، حجمه، أقسامه train/validation/holdout بلا تسرب جلسة بين الأقسام، ومصادر عناوينه (قرارات، بلاغات فائتة، عينة سلبية، مجموعة خلاف، مجموعة حدود).
3. **المرشحون:** ما الذي تغيّر (برومبت/نموذج/قواعد/عتبات/تصنيف)، نتائج التقييم لكل فئة (لا رقم إجمالي وحيد)، نتائج تشغيل الظل (فروق المرشح مقابل الإنتاج على الجلسات نفسها دون مساس بقائمة الإنتاج)، وحالة تحكيم عينة الاختلافات.
4. **سجل الإصدارات:** كل إصدار كاشف بأختامه الكاملة (Dataset version، Prompt SHA، Model ID، Thresholds، Taxonomy version)، قرار الترقية الموثق وموقّعه، حالة الإطلاق التدريجي (Canary)، وزر **تراجع فوري** إلى الإصدار السابق دون فقد أي قرار مراجعة (admin) [DECISION owner 2026-08-03, Amendment §6.5].
5. **العينة السلبية الأسبوعية:** حالة السحب الطبقي (برنامج، مستشار، مدة، شهر، جودة ترانسكربت، فئات قليلة التمثيل) وتغطية مراجعتها.

**قاعدتان ظاهرتان في رأس الشاشة:** «لا تعلم فورياً من قرار مراجع منفرد» و«أسماء المستشارين وتقييماتهم ليست ميزات كشف» — بنص ثابت [DECISION owner 2026-08-03, Amendment §6.1 و§6.7-5]. لا يوجد زر «تدريب فوري» في الواجهة أصلاً.

**الحالات:** فارغ: «لا دورة تعلم مكتملة بعد؛ تبدأ بعد أول مجموعة قرارات (VS-04).» · جزئي: تقييم أو ظل قيد التشغيل يعرض تقدمه ولا يعرض نتائج ناقصة كنهائية · خطأ: قياسية.

**من يراها:** §2.6 (SCR-18)؛ إيقاع الإصدار [ASSUME OD-33]. **التدقيق:** `DETECTOR_PROMOTION_DECIDED` / `DETECTOR_ROLLBACK_EXECUTED`.

### 15.6 SCR-19 — مركز الإجراءات (CAP-OPS-09 · ر16 · VS-06)

**الغرض:** ألا تقف المنصة عند عرض المؤشرات: كل نتيجة يمكن أن تصبح إجراءً له مالك وموعد وحالة ودليل إنجاز وقياس أثر لاحق [DECISION owner 2026-08-03, Amendment §2.4].

**العناصر:**

1. **قائمة الإجراءات:** نص الإجراء، المنشأ (رابط إلى النتيجة/البلاطة/قسم الإنفوجرافيك/الحالة المعتمدة التي ولّدته)، المالك، الموعد، الحالة (`open / in_progress / done / cancelled / overdue`)، المؤشر المستهدف وخط أساسه المسجل لحظة الإنشاء.
2. **إنشاء إجراء** بزر «إنشاء إجراء» من شاشات النتائج (SCR-02/13/20/21/22) — يحمل المنشأ تلقائياً.
3. **بطاقة الإجراء:** الوصف، أحداث إلحاقية (تكليف، تحديث حالة، تعليق)، دليل الإنجاز عند الإغلاق، وعند حلول موعد إعادة القياس: **مقارنة المؤشر قبل/بعد** بصياغة «تغيّر المؤشر بعد الإجراء» — **لا «بسبب الإجراء»** (قاموس §2.4؛ المنهج في الوثيقة 10) [DECISION owner 2026-08-03, Amendment §12-10].
4. **العرض التجميعي للقيادة:** نسبة الإنجاز `action_completion_rate`، الإجراءات المتأخرة، وملخص الأثر — دون تفاصيل اتهامية.

**الحالات:** فارغ: «لا إجراءات بعد — تُنشأ من شاشات النتائج بزر «إنشاء إجراء».» · جزئي: إجراء لم يحل موعد إعادة قياسه يعرض «قياس الأثر في {تاريخ}» بدل مقارنة ناقصة · خطأ: قياسية.

**من يراها:** §2.6 (SCR-19)؛ مالك الخدمة يملك الإجراء ويغلقه والقيادة ترى التجميعي [ASSUME OD-34]. **التدقيق:** `ACTION_CREATED` / `ACTION_STATUS_CHANGED`.

### 15.7 SCR-20 — الإنفوجرافيك الشهري (مسودة/اعتماد/نشر) (CAP-OPS-08 · ر15 · VS-05)

**الغرض:** إنتاج واعتماد ونشر **«نبض خدمة الاستشارات والإرشاد — ملخص الشهر»**: منتج قيادي مستقل أحادي الصفحة، يُرسم برمجياً من بيانات منظّمة حصراً — لا نموذج توليد صور، ولا أي رقم من نموذج لغوي (صياغة الجمل الموجزة بالنموذج جائزة بعد بوابة R6 فقط) [DECISION owner 2026-08-03, Amendment §7 + SD-19].

**العناصر:**

1. **قائمة الإصدارات الشهرية** بحالتها في الدورة الحرفية: `draft → data_review → content_review → approved → published → superseded/retracted`؛ إنشاء المسودة آلي بعد إقفال الشهر واجتياز بوابة الاكتمال (≥98% من الجلسات في حالات نهائية، أو صباح اليوم الثاني من الشهر الجديد — أيهما لاحقاً)؛ أي تجاوز (Override) لنقص الاكتمال **يظهر على وجه الإنفوجرافيك** لا في هامشه [DECISION owner 2026-08-03, Amendment §7.3].
2. **المعاينة أحادية الصفحة بالأقسام الخمسة** [DECISION owner 2026-08-03, Amendment §7.4]:
   - **نبض الخدمة:** الجلسات المكتملة، التغير عن الشهر السابق، الإلغاء وعدم الحضور، متوسط تقييم المستفيد وتوزيعه، نسبة اكتمال البيانات وربط المصدرين.
   - **جودة وأثر الجلسات:** نسبة الجلسات المنتهية بخطوات واضحة، توزيع الأثر (عالٍ/متوسط/منخفض) وفق المنهج المعتمد، أكثر 3 نقاط قوة متكررة، أكثر 3 فرص تحسين متكررة.
   - **صوت المستفيد:** أهم التحديات والأسئلة المتكررة، أهم مؤشرات الرضا وعدم الرضا، موضوع صاعد يستحق الانتباه، مقارنة مختصرة بالشهر السابق.
   - **الجودة والالتزام:** **المخالفات المعتمدة بشرياً فقط حسب النوع**؛ وعدد حالات الاشتباه الجديدة وقيد المراجعة والمتأخرة كأرقام قائمةٍ منفصلة معلَّمة «اشتباه — غير معتمد»؛ **لا أسماء مستشارين ولا تفاصيل اتهامية في النسخة القيادية العامة** [ASSUME OD-31].
   - **ما يحتاج قراراً:** أهم 3 مخاطر أو إشارات تشغيلية، أهم 3 إجراءات مقترحة للشهر التالي، مالك كل إجراء إن كان معتمداً، وحالة إجراءات الشهر السابق وقياس مختصر لأثرها (من SCR-19).
3. **التذييل الإلزامي** [DECISION owner 2026-08-03, Amendment §7.4]: الفترة؛ آخر تحديث؛ عدد الجلسات المستخدمة والمطابقة والإجمالي؛ الاستبعادات المهمة؛ إصدارات التصنيفات؛ لقطة المتن (corpus snapshot)؛ رابط التعمق (Drill-down).
4. **فحوص إلزامية قبل كل انتقال حالة:** بوابة الاكتمال أو Override ظاهر؛ اجتياز R6 لكل جملة مصاغة؛ خلو رقم «المخالفات» من أي حالة غير معتمدة؛ صحة الفترات بنهاية غير مشمولة.
5. **أدوار الدورة:** أمين البيانات يوقّع `data_review` (التغطية والأرقام)؛ مدير الخدمة يوقّع `content_review` (المعنى والتوصيات)؛ سلطة النشر تعتمد وتنشر [ASSUME OD-10]؛ وأي إصلاح بعد النشر نسخة جديدة `superseded` أو سحب `retracted` — المنشور لا يُعدَّل أبداً.
6. **النشر:** ثلاث صيغ من حمولة منظّمة واحدة **متطابقة بالبِت**: صفحة داخلية + PDF أحادي الصفحة + PNG عالية الدقة، برابط ثابت للنسخة المنشورة، عبر القنوات المعتمدة (البوابة + بريد معتمد) [ASSUME OD-29]؛ إعادة التوليد من الـartifact المنظّم لا من كشط DOM (I11).
7. **كل مؤشر قابل للنقر** → تعمق إلى الجلسات المستخدمة ضمن صلاحيات الناقر.

**معايير القبول** (تُنقل إلى الوثيقة 15 حرفياً): كل رقم يعود إلى Metric Result ومجموعة جلسات؛ لا أرقام مولّدة بنموذج؛ الفترات نهاية غير مشمولة؛ غير المعتمد لا يدخل رقم «المخالفات» المنشور؛ تطابق الصيغ الثلاث بالبِت في الحمولة؛ قابلية القراءة على شاشة قيادية وطباعة A4 [DECISION owner 2026-08-03, Amendment §7.7].

**الحالات:** فارغ: «لا إصدارات بعد؛ تنشأ المسودة آلياً بعد إقفال الشهر وبوابة الاكتمال.» · جزئي: مسودة بتجاوز اكتمال تعرض لافتة التجاوز على وجه الصفحة (حالة أولى الدرجة) · خطأ: قياسية.

**من يراها:** §2.6 (SCR-20). **التدقيق:** `INFOGRAPHIC_APPROVED` / `INFOGRAPHIC_PUBLISHED` / `INFOGRAPHIC_SUPERSEDED` / `INFOGRAPHIC_RETRACTED`.

### 15.8 SCR-21 — صباحيات الخدمة (CAP-OPS-10 · VS-03+)

**الغرض:** إجابة «ماذا حدث أمس؟» في دقيقتين دون فتح كل لوحة — رسالة/صفحة موجزة تصدر بعد التشغيل الليلي [DECISION owner 2026-08-03, Amendment §11.1].

**العناصر (كل بند: رقم + رابط مباشر إلى شاشته):**

1. الجلسات الجديدة (→ SCR-16 بفلتر الأمس).
2. حالات الاشتباه الجديدة (→ SCR-08؛ معلَّمة «مشتبه»).
3. الحالات المتأخرة في المراجعة (→ SCR-08 بفلتر SLA).
4. تقييمات 1–2 نجمة الجديدة (→ SCR-22).
5. جلسات انتهت بلا خطوات واضحة (→ SCR-22/SCR-16).
6. مشكلات DataHub/Read.ai (→ SCR-14 وSCR-11).
7. أهم تنبيه أو انحراف واحد (من قواعد التنبيه — الوثيقة 19).

**التسليم:** داخل المنصة + بريد معتمد [ASSUME OD-29]؛ أرشيف يومي على `/digests/{date}`؛ التوليد خطوة من خطوات التشغيل الليلي (الوثيقة 09) والواجهة قارئ له.

**الحالات:** فارغ: يومٌ بلا بنود يصدر رسالة مع ذلك: «لا بنود تحتاج تدخلاً اليوم» — إثباتاً للنضارة لا صمتاً ملتبساً · جزئي: تشغيل `partial` يجعل رأس الصباحية: «بيانات ليلة أمس غير مكتملة — {الفجوة المسماة}» · خطأ: قياسية.

**من يراها:** §2.6 (SCR-21) — قناة القيادة الشهرية هي الإنفوجرافيك، لا الصباحيات. **التدقيق:** `DIGEST_VIEWED`.

### 15.9 SCR-22 — قائمة التعافي الخدمي (CAP-OPS-11 · موجة B — تدخل التنفيذ بعد مشاورة SD-21)

**الغرض:** التقاط الحالات التي قد تحتاج تدخلاً خدمياً سريعاً (استرضاء، تصحيح، متابعة) — **هذه ليست مخالفات ولا اتهامات**، والقائمة مفصولة تماماً عن قائمة «اشتباه مخالفة»: بيانات مستقلة، صلاحيات مستقلة، تصميم مستقل، ولا إحالة بصرية بينهما [DECISION owner 2026-08-03, Amendment §11.2].

**مصادر الإدراج (قواعد محكومة معلنة بإصدار — لا استنتاج نموذج حر):** تقييم منخفض (1–2)؛ ملاحظة مستفيد سلبية صريحة؛ جلسة بلا خطوات واضحة؛ ضغط أو ارتباك مرتفع في نهاية الجلسة (من مؤشرات الإثراء المعتمدة)؛ جلسة أُلغيت مرات متعددة؛ توصية متابعة دون متابعة مسجلة.

**العناصر:** قائمة الحالات: سبب الإدراج وقاعدته وإصدارها، الجلسة (→ SCR-15)، حالة المتابعة (`new / in_follow_up / closed`)، مالك المتابعة؛ إغلاق موثق بإجراء (تواصل، تصحيح، إحالة، لا إجراء مبرَّر)؛ زر «إنشاء إجراء» (→ SCR-19) للحالات التي تحتاج معالجة نظامية. لا يظهر في هذه الشاشة أي مؤشر مخالفة حتى لو وُجد على الجلسة نفسها — الفصل كامل.

**الحالات:** فارغ: «لا حالات تعافٍ مفتوحة.» · جزئي: تأخر مصدر التقييمات يُعلن في رأس القائمة («تقييمات {تاريخ} لم تصل بعد») · خطأ: قياسية.

**من يراها:** §2.6 (SCR-22). **التدقيق:** `RECOVERY_CASE_CLOSED`.

---

## 16. القرارات المفتوحة والتوصيات ومحفزات إعادة النظر
...
| SLA المراجعة والإسناد (OD-30 — جديد بتعديل المالك) | 7 أيام للعادية ويومان لعالية الأولوية؛ الإسناد لقائد المراجعين [ASSUME OD-30] | SLA موحد بلا أولويات | قرار المالك مع بيانات أول شهر تشغيل |
| أسماء المستشارين في الإنفوجرافيك وأسطح التدريب (OD-31 — جديد) | محجوبة في النسخة العامة؛ المدير المباشر فقط وفق OD-09/OD-14 [ASSUME OD-31] | كشف موسع لملاك الخدمة | قرار المالك |
| قنوات نشر الإنفوجرافيك (OD-29 — جديد) | البوابة + بريد معتمد [ASSUME OD-29] | إضافة Teams/قناة داخلية | اعتماد قناة جديدة رسمياً |
| ملكية مركز الإجراءات وسلطة الإغلاق (OD-34 — جديد) | مالك الخدمة يملك ويغلق؛ القيادة تجميعي [ASSUME OD-34] | إغلاق بتوقيع مزدوج | قرار المالك عند تشغيل SCR-19 |
| أول نوع مخالفة رأسي (OD-35 — جديد) | VIOL-008 «تواصل خارج الإطار الرسمي» [ASSUME OD-35] | نوع آخر يختاره المالك | قبل بناء VS-02 |

~~**تسليم إلى الوثيقة 21:** ترتيب البناء الموصى به للشاشات: المكونات المشتركة (§2.3) ثم SCR-01/02/04 (المحادثة والإجابة والتوضيح — الحد الأدنى القابل للعرض)، ثم SCR-03 (الأدلة)، ثم SCR-08/09 (حلقة المراجعة الواحدة — مبكراً لأنها تخدم مفتاح الإجابات أيضاً [DECISION MASTER §4.4-3])، ثم SCR-05/06/07 (Lane 3)، ثم SCR-10/11/12 [REC].~~ **حلّت محل هذه التوصية — التي كانت تقدّم المحادثة أولاً — ترتيبُ البناء الملزم في §17 أدناه** [DECISION owner 2026-08-03, Amendment §12-18]؛ بقيت مقروءة هنا توثيقاً للإطار السابق فقط.

---

## 17. ترتيب البناء الملزم للشاشات (تعديل المالك §12-18)

الترتيب التالي **ملزم** ويحل محل أي ترتيب سابق في هذه الوثيقة [DECISION owner 2026-08-03, Amendment §12-18]:

1. **SCR-14 مركز تشغيل البيانات** — لا منتج بلا رؤية تشغيل.
2. **SCR-15 الجلسة 360** (وبوابتها SCR-16 بحث الجلسات) — أول جلسة موحدة من المصدرين.
3. **SCR-08a قائمة «اشتباه مخالفة»** — أول قيمة مراجعة.
4. **SCR-08b الحالة التفصيلية** بقراراتها الست.
5. **SCR-17 الإبلاغ عن اشتباه فائت** — لقياس ما فات لا ما اكتُشف فقط.
6. **SCR-18 لوحة جودة الكاشف والتعلم.**
7. **SCR-13 لوحة تشغيل الخدمة** (اللوحة التشغيلية الأولية CAP-OPS-12).
8. **SCR-19 مركز الإجراءات.**
9. **SCR-20 الإنفوجرافيك الشهري.**
10. **المحادثة التحليلية (SCR-01/02/04 ثم SCR-05/06/07) لاحقاً — فوق القدرات المعتمدة حصراً.**

**جملة حاكمة:** **المحادثة التحليلية تأتي بعد ثبات النواة التشغيلية** — التشغيل والمراجعة أهم لإثبات قيمة النظام، والوكيل واجهة فوق قدرات ثبتت لا مركز المنتج [DECISION owner 2026-08-03, Amendment §12-18 + §12-12].

ملاحظات تنفيذ: (أ) المكونات المشتركة §2.3 تُبنى أولاً تقنياً لأن كل الشاشات تستهلكها — وأولها `ScopeCoverageBlock` و`StampsBar` [REC]؛ (ب) هذا ترتيب شاشات؛ ربط كل شاشة بشريحتها ومستوى تعرضها (VS-00…VS-12 / L0–L3) تملكه الوثيقتان 21 و25 — فمثلاً تظهر نسخة قراءة مبكرة من SCR-14 مع VS-00/VS-01 (صفحة حالة فعلية لا Hardcoded) وتكتمل بأدوات الإجراءات في VS-03؛ (ج) SCR-09/SCR-10/SCR-11/SCR-12 تبقى مواقعها مربوطة بحلقات الحوكمة والنشر في شرائحها (SCR-09 مبكرة نسبياً لخدمة إعادة التصنيف ومقترحات «نوع جديد» من SCR-17)؛ (د) كل شاشة تُعرض للمالك في معاينة L1 فور اكتمال شريحتها — بوابات الإنتاج تحكم L2/L3 لا معاينة L1 (SD-18) [DECISION owner 2026-08-03, Amendment §9.2].
