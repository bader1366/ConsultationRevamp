...
**Depends on:** 03 (الشخصيات والأدوار), 04 (كتالوج القدرات), 08 (نموذج البيانات), 11 (الطبقة الدلالية), 12 (خدمة المسارات والنصوص الثابتة), 13 (مصنع التحليل المعمّق والترقية), 15 (مفتاح الإجابات), 16 (الأمن والخصوصية), 17 (عقود API) · **Feeds:** 21 (خطة التسليم), 22 (سجل القرارات), 23 (حزمة التنفيذ)
**Sources used:** GREENFIELD §4, §5-B4, §6.1–6.5, §8.8, §12.7, §14.1–14.7, §15.1–15.4, §16.2–16.3, §23; MASTER_PROMPT §4.4, §5.0–5.3, Appendix C; CORE-BRIEF §3–§6, §10–§13; doc 12 §12 (النصوص العربية الثابتة لرموز الأسباب); doc 13 §8.4 (عتبات الترقية)

---

## 1. الغرض والنطاق ومبادئ التصميم

تحدد هذه الوثيقة مواصفة واجهات الاستخدام العربية للمنصة، شاشةً بشاشة، للمسارات الاثني عشر الإلزامية المذكورة في GREENFIELD §16.2 [FACT GREENFIELD §16.2]. الوثيقة قابلة للتنفيذ مباشرة: لكل شاشة غرضها، وعناصرها، وحالاتها (فارغ/جزئي/خطأ)، وقواعد صياغتها العربية، ومتطلبات الوصول واتجاه الكتابة، ومن يراها حسب الدور. عقود JSON التفصيلية للنداءات تملكها الوثيقة 17؛ ما يرد هنا من مسارات `/v1/…` هو إشارة توافقية لا نصاً معيارياً [INFER].
...
2. **النطاق قبل الإجابة وفيها** — كل إجابة تحليلية تُسبق وتُرافق بكتلة النطاق والتغطية: الفترة المقررة ومصدر قرارها، الفلاتر، عدد الجلسات، التغطية، مصادر التفريغ (I4، I8).
...
| الأرقام في البيانات | أرقام لاتينية (0–9) في كل الجداول والبطاقات والمحاور والتصديرات، بخط ذي أرقام جدولية (tabular figures) | [REC — البديل: الأرقام الهندية (٠–٩)؛ رُجحت اللاتينية لاتساقها مع XLSX والمعرفات والفروق الحسابية؛ محفز إعادة النظر: طلب صريح من مالك المنتج للحزم التنفيذية — انظر OD-21] |
...
| العلامة الجزئية | كل رقم قائم على n < 30 يُستبدل بحالة «بيانات غير كافية (n={n})» — لا يُعرض الرقم أصلاً | [DECISION CORE-BRIEF §13-4] |
...
| أي ذكر لاسم مستفيد أو هوية وطنية | العنصر النائب الثابت («المستفيد م-1042») | I15 |
...
| SCR-08 مراجعة المخالفات | ✗ | ✗ | ◐ مؤشرات معتمدة مجمعة فقط | ✓ | ◐ قراءة | ✓ |
...
الشريط الجانبي الرئيس يعرض للمستخدم أقسامه المصرح بها فقط (المحادثة، اللوحات، المهام، المراجعة، الحوكمة، النشر، صحة البيانات) — والإخفاء تحسين تجربة لا ضبطاً أمنياً؛ الضبط خادمي دائماً (I14).
...
| `REVIEW_DECISION` | SCR-08 | `case_id`, القرار, الفئة, `finding_fingerprint`, المبرر |
...
| `DASHBOARD_VIEWED` | SCR-13 | `dashboard_id`, البلاطات المعروضة, الفترة المحسوبة, أختام التصنيفات |

حدث القرار في SCR-08 يخزن **ما رآه المراجع** (البصمة والنسخة) لا فقط ما قرره — هذا ما يجعل الإبطال الآلي عند تغير الأساس قابلاً للتنفيذ (§10).
...
**نقاط الخدمة:** `POST /v1/chat/turns` (إنشاء لفة)، `GET /v1/capabilities` (رقاقات الاقتراح) — العقود في الوثيقة 17.
...
> **لفة 4:** «وماذا عن الجودة؟» (صيغة خارج الجدول المغلق) → النص الثابت لـ `AMBIGUOUS_REFERENCE`: «لم أتمكن من ربط هذه الإشارة بسؤال سابق في المحادثة. الرجاء إعادة صياغة السؤال كاملاً.» — لا تخمين أبداً [DECISION MASTER §5.0].
...
**نقاط الخدمة:** `GET /v1/evidence/{finding_id}/window` (نافذة لفات ±N)، يسجل حدث `EVIDENCE_ACCESS` في `ops.audit_event` قبل إرجاع النص [DECISION GREENFIELD §15.4].
...
## 6. المسار 4 — السؤال التوضيحي المغلق (SCR-04: بطاقة التوضيح)
...
**نقاط الخدمة:** القبول يستدعي `POST /v1/jobs` (الأداة `start_deep_analysis` يتحكم بها المنسق لا النموذج — doc 12)؛ المتابعة `GET /v1/jobs/{job_id}`؛ الإيقاف `DELETE /v1/jobs/{job_id}`.
...
2. **جدول النطاق قبل القبول:** الأقسام الشهرية (مثال: «2026-01: 1,102 جلسة · 2026-02: 987 · 2026-03: 1,215»)، إجمالي استدعاءات النموذج المقدرة، المدة التقديرية («~ 40–70 دقيقة»)، الإنفاق التقديري («~ {est} ريال/دولار حسب تسعيرة النموذج المسجل») — يُعرض ولا يُشترط قبوله بحد [DECISION GREENFIELD §2.5].
...
## 10. المسار 8 — مراجعة المخالفات المشتبهة (SCR-08a: طابور الانتظار · SCR-08b: بطاقة الحالة)

**الغرض:** الحلقة الواحدة «يقترح الآلي ويفصل البشري» على أخطر مخرجات المنصة: مؤشرات المخالفات (CAP-B4/B5). التاريخ **إلحاقي فقط لا يُحذف** (append-only)، والقرارات تُبطل آلياً عند تغير الأساس الذي بُنيت عليه [DECISION GREENFIELD §4.3 + §6.5؛ ADR-0012].

### SCR-08a — طابور الانتظار
...
مفردات الكشف الخمس (أسلوب غير مهني، تضليل، معلومات غير مؤكدة، تهكم، تسويق شخصي) تُعرض في فلتر مستقل «مفردات الكشف» موصولة بجدول المطابقة في الوثيقة 04، مع تعليم فجوتي التغطية (تهكم، تسويق شخصي) بشارة «لا كاشف نشط» [FACT CORE-BRIEF §13-2].

عناصر الطابور:

1. أعمدة: عمر الحالة (SLA بادٍ بالألوان: > 7 أيام تحذير، > 14 حرج [REC])، الفئة المقترحة (`VIOL-001…008` بأسمائها أعلاه)، درجة الفرز الأولي (للترتيب فقط — لا تُعرض كصحة؛ I18)، المستشار (اسم مستعار حتى فتح البطاقة)، تاريخ الجلسة، الحالة (`pending / in_review / approved / rejected / needs_second_review / stale`).
2. فلاتر: الفئة، الفترة، الحالة، «حالاتي المسندة».
3. ترتيب افتراضي: الأقدم أولاً (مكافحة تراكم SLA) [REC].

### SCR-08b — بطاقة الحالة
...
├───────────────────────────────┴──────────────────────────────┤
│ [اعتماد] [رفض] [إحالة لمراجعة ثانية] [تصعيد]                  │
│ الفئة: [VIOL-005 ▾]  المبرر: [____________ ≥15 حرفاً]          │
├──────────────────────────────────────────────────────────────┤
...
1. **الاقتباس الحرفي مُبرزاً** داخل نافذة سياق ±5 لفات (عارض SCR-03 مضمّن) — لا قرار بلا نص كامل أمام العين.
2. **لوحة الهوية والمنشأ:** هوية الجلسة كاملة + `extraction_run_id` و`model_id` و`prompt_sha` و`taxonomy_version` و`source_version` — أي قرار يُختم بهذه **البصمة** (`finding_fingerprint`) لحظة اتخاذه.
3. **صندوق القرار:** أزرار: «اعتماد» / «رفض» / «إحالة لمراجعة ثانية» / «تصعيد للمدير». الاعتماد والرفض يتطلبان: تأكيد الفئة (أو إعادة تصنيفها ضمن `VIOL-*` أو «ليست مخالفة — {سبب مغلق: مشكلة تقنية / سياق مقبول / اقتباس خاطئ / أخرى بمبرر}»)، ومبرراً نصياً ≥ 15 حرفاً.
4. **سجل القرارات (إلحاقي):** كل الأحداث السابقة بترتيب زمني: من، متى، ماذا، البصمة المختوم عليها — **لا زر حذف ولا تعديل**؛ التراجع = قرار جديد يعقب القديم ويعلوه، والقديم يبقى مقروءاً [DECISION GREENFIELD §16.2-8؛ ASSUME OD-11: الحذف النهائي (retire) للمدير حصراً، وحتى هو إلحاقي التسجيل].
5. **لافتة الإبطال (staleness):** إذا تغيرت المستخرجات بعد القرار (rebase للتفريغ، إعادة استخراج بنموذج/موجه جديد، تعديل تصنيف يمس الفئة) تتحول الحالة آلياً إلى `stale` وتعود للطابور بلافتة: «قرار سابق مؤرخ 2026-06-20 اتُّخذ على بصمة مستخرجات أقدم (v2). أعد المراجعة على الأساس الحالي (v3).» — القرار القديم لا يُحذف ولا يبقى سارياً [DECISION GREENFIELD §4.3].
6. **صلاحيات حسب الدور [ASSUME OD-11]:** reviewer: اعتماد/رفض/إحالة (رفضه نافذ — الافتراض الآمن أن المراجع غير الإداري يملك الرفض مع بقائه قابلاً للنقض الإلحاقي من admin)؛ steward: قراءة فقط؛ admin: كل ما سبق + `retire`؛ service_owner لا يرى إلا مؤشرات معتمدة **مجمعة** بلا أسماء.

**مثال بطاقة (توضيحي):** الفئة المقترحة: `VIOL-005 — طرح رأي شخصي كحقيقة` (v3) · الاقتباس: «أنا متأكد إن هالنشاط ما له مستقبل في السوق، لا تكمل فيه» — دور المتحدث: مستشار، اللفة 143 · القرار: اعتماد، المبرر: «تقرير حاسم بلا مصدر أو تحوط، يطابق تعريف الفئة».
...
5. **إعادة الإصدار (الإحلال):** «إصدار نسخة محلّة» ينشئ حزمة جديدة تشير للقديمة بسلسلة إحلال ظاهرة: «الإصدار 2 (2026-07-10) يحل محل الإصدار 1 (2026-07-01) — سبب الإحلال: {رمز مغلق: taxonomy / corpus / code / behaviour} + بيان». القديمة تبقى مقروءة بلافتة «أُحلّت — انظر الإصدار الأحدث».
...
**الحالات:** فارغ: «لا حزم لهذه الفترة بعد؛ التجهيز الآلي يبدأ بعد إقفال الفترة بـ {n} أيام.» · جزئي: مسودة على بيانات ناقصة تُعرض بفجواتها ولا تتقدم · خطأ: قياسية.
...
**من يراها:** §2.6 (SCR-10)؛ executive_viewer يرى المنشور والمُحل والمسحوب بلافتاتها — لا المسودات.
...
3. **بنية PDF:** غلاف بالأختام + `ViewBadge` (مجمّد/حي) + ترويسة على كل صفحة تحمل `export_id` والفترة؛ خاتمة «حدود الاستخدام»: «أعد هذا الملف من منصة نوافث؛ الأرقام مرتبطة بأختام الإصدارات المدونة؛ مؤشرات المخالفات غير المعتمدة ليست وقائع مثبتة».
...
- **مصدر مسحوب:** طلب تصدير من حزمة مسحوبة يُمنع بنص: «هذه الحزمة مسحوبة ({retraction_id}) ولا يتاح تصديرها؛ انظر الإصدار المحلّ.»
...
**المسار:** `/dashboards` (اللوحة الافتراضية للدور) · `/dashboards/{dashboard_id}` (لوحة محددة). التكوين لكل شخصية [REC]:
...
| admin | كل ما سبق + صحة خطوط الأنابيب والإنفاق النموذجي (الوثيقة 19) |
...
2. **الفترة المحسوبة** ظاهرة على رأس اللوحة وعلى كل بلاطة تخالفها؛ لا حالة «كل الفترات» ضمنية أبداً (I4).
3. **فترات الثقة Wilson وقمع n<30**: الخلية دون العتبة تعرض العدد وتحجب النسبة بعلامة «عينة غير كافية» — لا استثناء للوحات (قواعد المنهجية، الوثيقة 10).
4. **أختام إصدارات التصنيف** على كل بلاطة تعرض عدّاً مصنفاً (I16)، وشارة «عرض حي» تمييزاً عن أرقام الحزم المجمدة (SCR-10).
