...
## 11. Operational/product capabilities (CAP-OPS series)

[DECISION owner 2026-08-03 / Amendment §2, §12 doc 04 item] The Owner Amendment re-centres the plan on an operational product: «لا تقتصر القدرات على الأسئلة التحليلية». This section registers the twelve operational/product capabilities **CAP-OPS-01…CAP-OPS-12** under the same registry discipline as CAP-A/B/C/D (I12: one governed registry; an unregistered surface fails the build). Settled decisions cited throughout — doc 02 owns the register: **SD-18** incremental vertical-slice delivery with a mandatory stop after every slice; **SD-19** the monthly leadership infographic as a standalone early product; **SD-20** the violation-suspicion queue as an early product with finding/case/event separation; **SD-21** backlog governance (consultation-first); **SD-22** governed detector learning, never online self-learning.

### 11.1 Registry mechanics for the OPS series

- **Same registry family, sibling meta-schema** [REC]. OPS entries live as `registry/capabilities/CAP-OPS-*.yaml`, CI-validated and loaded into `serve.capability_registry` exactly like every other entry. The §1.2/§1.3 id patterns widen to `^CAP-([ABCD][0-9]{1,2}|OPS-[0-9]{2}|E[0-9]{2})$` (also regularizing the CAP-E promotion series of §9.2); the `entry` payload is validated against the meta-schema matching its series — analytical entries keep §1.2 unchanged; OPS entries use the field list below. Alternative — a separate ops registry — rejected: two registries is the F5/F8 defect reborn. Revisit-trigger: none; I12 settles it.
- **OPS entries are product/operational capabilities, not Lane-0 Q&A entries.** They carry no τ/δ, no paraphrase sets, no answer key of their own; they are served through their own screens (doc 18 — SCR-14…SCR-22 plus the upgraded SCR-08 «اشتباه مخالفة») and APIs (doc 17), behind the amendment's exact feature flags, under server-side RBAC (doc 16). Where the Lane-1 agent reads their state it does so only through governed read-only tools (doc 12); the agent never creates or approves a violation, never publishes an infographic, and never creates or closes an action outside the authorized workflow [DECISION Amendment §12 doc 12 item].
- **OPS entry field list (all required; "not applicable" is explicit, never omitted — R14):** `capability_id` (`CAP-OPS-NN`) · `slug` · `display_name_ar` · `personas` (RBAC enum of §1.2; the amendment's new doc-03 personas — قائد المراجعين/Adjudicator، Data Steward، Operations Engineer، AI/Model Owner — map onto reviewer/steward/admin pending the doc 16 role extension) · `owner_role` (accountable operating persona) · `kind: operational` · `source_data` (schemas/tables + upstream contracts incl. SRC-DATAHUB sub-feeds, doc 05) · `acceptance_ref` (amendment-§ criteria distilled in §11.3; executable checklists live in doc 25) · `dependencies` · `feature_flag` (exact amendment names only) · `release_slice` (VS-00…VS-12) · `wave` (Wave A/B/C — doc 24 owns the backlog; waves are NOT doc 21's P0…P7 phase names) · `backlog_ids` (PB-xxx) · `screens` (SCR-xx) · `ops_metrics` (registered ids, §11.4) · `audit_obligations`.

### 11.2 Summary matrix — CAP-OPS-01…12

| ID | slug | Arabic name | Primary personas | Owner role | Feature flag | Slice | Wave | Backlog |
|---|---|---|---|---|---|---|---|---|
| CAP-OPS-01 | `nightly_consolidation_run` | التشغيل الليلي الموحد | steward, admin | Operations Engineer + Data Steward | `nightly_consolidation_enabled` | VS-03 | A | PB-001 |
| CAP-OPS-02 | `data_ops_center` | مركز تشغيل البيانات | steward, admin, service_owner | Operations Engineer | `nightly_consolidation_enabled` | VS-03 | A | PB-002 |
| CAP-OPS-03 | `session_360` | الجلسة 360 | service_owner, reviewer, analyst | Service Owner | `session_360_enabled` | VS-01 | A | PB-004 |
| CAP-OPS-04 | `session_search` | بحث الجلسات وتصفيتها | service_owner, reviewer, analyst | Service Owner | `session_360_enabled` | VS-01 | A | PB-005 |
| CAP-OPS-05 | `violation_review_queue` | اشتباه مخالفة (قائمة وحالة تفصيلية) | reviewer, admin | Review lead (قائد المراجعين) | `violation_review_enabled` | VS-02 | A | PB-006, PB-007, PB-008, PB-009 |
| CAP-OPS-06 | `missed_violation_report` | إضافة اشتباه لم يرصده النظام | reviewer | Review lead | `violation_review_enabled` | VS-04 [REC — surface hosted in Session 360 from VS-02's review UX; consumed by the VS-04 learning loop] | A | PB-010 |
| CAP-OPS-07 | `detector_learning_pipeline` | حلقة تحسين الكاشف | admin (SCR-18 surface); pipeline is worker-plane | AI/Model Owner | `violation_review_enabled` (UI surface); pipeline gated by SD-22 promotion approvals, not a runtime flag | VS-04 | A | PB-011 |
| CAP-OPS-08 | `monthly_infographic` | نبض خدمة الاستشارات والإرشاد — ملخص الشهر | executive_viewer, service_owner, admin | Publication Authority [ASSUME OD-10] | `monthly_infographic_enabled` | VS-05 | A | PB-014 |
| CAP-OPS-09 | `action_center` | مركز الإجراءات | service_owner, admin; executive_viewer (aggregate) | Service Owner [ASSUME OD-34] | `action_center_enabled` | VS-06 | A | PB-015 |
| CAP-OPS-10 | `morning_digest` | صباحيات الخدمة | service_owner, steward, admin | Service Owner | `nightly_consolidation_enabled` (digest is a nightly-run output, Amendment §4.2 step 15) | VS-06 [REC — minimal emission wired in VS-03] | A | PB-016 |
| CAP-OPS-11 | `service_recovery_queue` | قائمة التعافي الخدمي | service_owner | Service Owner | minted at owner approval per SD-21 ([REC] proposed name `service_recovery_enabled`) | VS-08 (enters only after SD-21 consultation) | B | PB-101 |
| CAP-OPS-12 | `ops_dashboard` | اللوحة التشغيلية الأولية | service_owner, executive_viewer, analyst | Service Owner | no dedicated amendment flag — KPI cards are core registry-governed serving; run-derived panels degrade under `nightly_consolidation_enabled` (I16, never hardcoded) | VS-06 | A | PB-013 |

### 11.3 Full entries — source data · acceptance criteria · dependencies

**CAP-OPS-01 · `nightly_consolidation_run` — التشغيل الليلي الموحد.** *Source data:* all SRC-DATAHUB sub-feeds + the active `TranscriptSource` (doc 05/06); writes `ops.pipeline_run` / `ops.pipeline_step_run` / `ingest.source_watermark` / DLQ ([REC placement; doc 08 owns final DDL per Amendment §12 doc 08 item]). *Behaviour:* the 16 mandatory steps of Amendment §4.2; run states exact `queued → running → partial → succeeded → failed → cancelled → superseded`; starts 02:00 Asia/Riyadh, configurable, hourly Read.ai incremental allowed but nightly is authoritative [ASSUME OD-28]. *Acceptance (distilled Amendment §4.6):* 20 Read.ai + 20 DataHub sample records produce the correct Session-360 count without duplication; re-running the same window twice changes no counts and creates no duplicate review cases; a DataHub-evaluation sub-feed failure yields `partial` while transcript ingest still succeeds; a late-arriving evaluation updates the session in the next run; a generated suspicion case appears in the queue and opens the correct text at the quote; the dashboard shows watermarks/coverage/errors from real data, never hardcoded. Idempotency, per-adapter watermarks, DLQ-with-reason, no silent success, `rerun failed items` / `reprocess selected sessions` commands, no local-JSON state (Amendment §4.4). *Dependencies:* SRC-DATAHUB contracts (doc 05, OD-07 extension), doc 09 nightly chapter. *Ops metrics:* `nightly_run_success`, `data_completeness`, `source_join_rate`.

**CAP-OPS-02 · `data_ops_center` — مركز تشغيل البيانات (SCR-14).** *Source data:* `ops.pipeline_run` family, reconciliation and DQ queues, watermarks. *Acceptance (Amendment §4.5):* shows last run + status; per-source status and last success; new/modified session counts; per-step durations; unmatched sessions; transcript-less sessions; new suspicion count; overdue reviews; current-month completeness; operator actions (retry, open issue, view logs, reprocess session, download run manifest) — every figure from live data (hardcoded values are a forbidden implementation per Amendment §10.4-7), every action audited. *Dependencies:* CAP-OPS-01. *Ops metrics:* all seven of §11.4 surface here.

**CAP-OPS-03 · `session_360` — الجلسة 360 (SCR-15).** *Source data:* `core.advisory_session` hub + crosswalks, active transcript, findings, `core.beneficiary_rating` / `core.consultant_evaluation`, DataHub session facts — with per-field source provenance per the doc 05 Field Authority Matrix. *Acceptance (distilled VS-01 + Amendment §3.3):* one session shows both sources with provenance and coverage; no duplication; crosswalk visible; unmatched sessions visible as such; re-ingest does not change counts; a beneficiary evaluation arriving 2 days late appears after the next run without a full rebuild; hosts the CAP-OPS-06 entry point. *Dependencies:* SRC-DATAHUB-SESSION/-BENEFICIARY-EVAL/-CONSULTANT-EVAL contracts; `advisory_session_id` + crosswalk (doc 08). *PII:* RBAC per doc 16; beneficiary PII placeholder rules (I15, OD-09) unchanged.

**CAP-OPS-04 · `session_search` — بحث الجلسات وتصفيتها (SCR-16).** *Source data:* `core.advisory_session` + dimension tables + rating/status facts. *Acceptance (PB-005):* search and filter by period (end-exclusive), programme, consultant, session status, rating; RBAC-scoped results; result rows open Session 360. *Dependencies:* CAP-OPS-03; registered dimensions only (I12 — no free-text filter reaches SQL, I2).

**CAP-OPS-05 · `violation_review_queue` — اشتباه مخالفة (upgraded SCR-08; no new SCR id).** *Source data:* `violation_finding` / `review_case` / `review_event` (append-only; doc 08 amendment entities), detector + taxonomy stamps, DataHub session context. *Behaviour:* display name «اشتباه مخالفة»; the fixed disclaimer verbatim (§4.7); list row fields and filters per Amendment §5.2 including SLA-overdue and reopened-by-change filters; detail view per §5.3 — quote with ≥5 turns context each side, direct jump into the full transcript, detection rationale (rule/model/prompt/threshold), similar approved cases with the anchoring guard (Amendment §6.7 rule 4: definition first, not many prior examples), append-only decision log, reviewed-findings fingerprint, stale-warning on transcript/extraction change. Six decisions with closed reason codes, reviewer identity, stamps, no overwrite (§5.4); finding/case/event separation with per-finding verdicts (§5.5). First vertical type **VIOL-008 التواصل خارج الإطار الرسمي**, then one type at a time [DECISION SD-20; ASSUME OD-35 — owner may swap before the VS-02 build]. *Acceptance (distilled Amendment §5.7):* every quote matches the active transcript literally; approval only by an authorized reviewer; an approved case leaves the suspected view on refresh; transcript change or re-extraction invalidates or re-opens the decision via fingerprint; one finding can be accepted and another rejected in the same session; every transition visible in the audit trail and undeletable; **no non-approved case enters any leadership number as a confirmed violation**. *Dependencies:* CAP-B4 detection substrate; review SLA + assignment [ASSUME OD-30: 7d normal / 2d high-priority; assignment by review lead]. *Ops metrics:* `suspected_cases_open`, `review_backlog_age`, `detector_acceptance_rate`.

**CAP-OPS-06 · `missed_violation_report` — إضافة اشتباه لم يرصده النظام (SCR-17).** *Source data:* writes `findings.missed_violation_report` + a linked review case; reads Session 360/full transcript. *Acceptance (Amendment §6.3):* from Session 360 or the full text the reviewer selects the span, chooses a type or «نوع جديد», states the reason, and the case routes to mandatory second review; «نوع جديد» proposals go to the taxonomy proposal queue (§7 candidate flow), never straight into VIOL. Without this feature the system measures only what it detected — it is the false-negative capture channel (doc 15 DS-13; MR-18). *Dependencies:* CAP-OPS-03 (host surface), CAP-OPS-05 (review flow).

**CAP-OPS-07 · `detector_learning_pipeline` — حلقة تحسين الكاشف (SCR-18 surface).** *Source data:* review labels + missed-case reports (DS-13) + weekly stratified random negatives (DS-14) + disagreement/adjudication set (DS-15) → versioned immutable `label_dataset` → `detector_candidate` / `detector_release` / `shadow_result` ([REC placement in `ops`; doc 08 owns]). *Behaviour (SD-22 verbatim chain):* dataset build → candidate (prompt/model/rules/threshold) → offline eval per category (precision AND estimated recall with CIs) → regression on stable categories → shadow → human adjudication → documented promotion → canary → full deploy with version stamp → monitoring → instant rollback. *Acceptance (distilled VS-04 + Amendment §6.5–6.7):* versioned dataset with separate holdout and zero session leakage; per-category results, never one aggregate number; promotion requires a documented human approval; rollback proven without losing review decisions; **no reviewer click ever changes production behaviour directly; consultant names/ratings are never detector features**. Release cadence monthly or on gate-pass, whichever is less frequent [ASSUME OD-33]. *Dependencies:* CAP-OPS-05/06 label flow; doc 14 release lifecycle; doc 15 §6.6 evidence standard + T-20. *Ops metrics:* `detector_acceptance_rate`.

**CAP-OPS-08 · `monthly_infographic` — نبض خدمة الاستشارات والإرشاد — ملخص الشهر (SCR-20; SD-19).** *Source data:* pack-grade Metric Results only (doc 11), month `corpus_snapshot`, approved-violations series (MR-17). *Behaviour:* one page; web + PDF + PNG with **byte-identical numeric payloads**; stable permalink; lifecycle `draft → data_review → content_review → approved → published → superseded/retracted` with Data Owner / Service Owner / Publication Authority roles (Amendment §7.5; reissue never edits); drawn programmatically from structured data ONLY — no LLM-computed numbers, no image-generation models; optional LLM phrasing passes the R6 numeric-provenance gate; content = Amendment §7.4's five parts + mandatory footer (period, last update, used/matched/total sessions, exclusions, taxonomy versions, corpus snapshot, drill-down link); one fixed template first (§7.6). Draft on the morning of day 2 of the new month or at completeness ≥98%, whichever comes later, with any override rendered on the face [ASSUME OD-28-adjacent — Amendment §13 working assumptions]; channels Portal + approved Email [ASSUME OD-29]; no consultant names in the general edition [ASSUME OD-31]. *Acceptance (distilled Amendment §7.7):* every number traceable to a Metric Result and session set; no LLM-generated numbers; end-exclusive comparison windows; unapproved violations never in the published violations figure; tri-format byte parity (doc 15 T-18); KPI click opens drill-down within RBAC; regenerated from the pack artifact, never DOM scraping (I11); readable on a leadership screen and A4 print. Doc 10 MR-20 governs KPI selection/suppression. *Dependencies:* CAP-OPS-01 month close; CAP-D10 pack machinery (this is a standalone product, not a CAP-D10 sub-renderer); metric registry. *Ops metrics:* `data_completeness` gate read.

**CAP-OPS-09 · `action_center` — مركز الإجراءات (SCR-19).** *Source data:* `service_improvement_action` / `action_event` / `action_metric_baseline` ([REC placement in `ops`; doc 08 owns]). *Behaviour:* finding → recommended action → owner → due date → status → completion evidence → post-action metric comparison (Amendment §2.4); append-only action events. *Acceptance (VS-06 journey):* discover a clear-steps drop, open its sessions, create a coaching action with owner and due date, update status, and later read the baseline→follow-up comparison in association-only wording (doc 10 MR-19 — no unproven causality). *Dependencies:* CAP-OPS-12 KPIs; ownership and closure authority [ASSUME OD-34: service owner owns actions; leadership sees aggregate status/impact]. *Ops metrics:* `action_completion_rate`.

**CAP-OPS-10 · `morning_digest` — صباحيات الخدمة (SCR-21).** *Source data:* nightly-run outputs only. *Acceptance (Amendment §11.1):* after a successful (or explicitly partial) nightly run, a digest lists new sessions; new suspicion cases; overdue reviews; new 1–2★ ratings; sessions ending without clear steps; DataHub/Read.ai issues; the top alert or deviation — each item deep-links to its screen; respects recipient RBAC; delivery failures are observable (doc 19), never silent. *Dependencies:* CAP-OPS-01; notification entities (doc 08 amendment list).

**CAP-OPS-11 · `service_recovery_queue` — قائمة التعافي الخدمي (SCR-22; Wave B).** *Source data:* governed rules over ratings, findings, status facts (low rating; explicit negative beneficiary comment; no clear steps; high end-of-session pressure/confusion; repeated cancellations; follow-up recommendation without recorded follow-up — Amendment §11.2). *Hard rule:* these are service-recovery signals, **completely separate from the violations queue** — never rendered with violation vocabulary. *Acceptance (PB-101):* governed rules + follow-up state + documented closure. *Dependencies:* enters an implementation slice only after explicit owner consultation and approval (SD-21).

**CAP-OPS-12 · `ops_dashboard` — اللوحة التشغيلية الأولية.** *Source data:* registered metrics only (doc 11 §8 + §8.4). *Acceptance (PB-013 / VS-06):* daily service volume and quality KPIs with drill-down and coverage blocks; suspected-vs-approved split obeyed on any violation tile (MR-17); zero hardcoded values (structural scan — Amendment §10.4-7); degradation states visible (I16). *Dependencies:* VS-07 metric set (doc 11 §8.3) for the first KPI row; doc 18 persona dashboards (existing SCR series) extended, no new SCR id minted.

### 11.4 Operational metric references (names exact)

The OPS series reports through these seven registered metrics — registered in **doc 11 §8.4** with base tables (`ops`/`jobs`/`findings`/`ingest` schemas), grains, and caveats; doc 19 alerts on them; an unregistered ops number on any screen fails the build (I12) [DECISION owner 2026-08-03 / Amendment §12 doc 04 item]:

`review_backlog_age` · `data_completeness` · `source_join_rate` · `action_completion_rate` · `detector_acceptance_rate` · `suspected_cases_open` · `nightly_run_success`

(The amendment's shorthand `join_rate` is registered under its exact package name `source_join_rate`.)

### 11.5 Suspected-vs-approved and SD-21 build order — catalogue-wide restatement

1. **Suspected ≠ approved, everywhere** [DECISION owner 2026-08-03 / Amendment §13]: suspected findings/cases appear only as queue-volume figures (`suspected_cases_open`, `review_backlog_age`); every published or leadership-facing violation count is approved-only; the two are never mixed in one series. CAP-B4/CAP-B5 entries carry the split explicitly (§4.7/§4.8); doc 10 MR-17 is the method rule; doc 15 T-19 is the pin.
2. **SD-21 governs what gets built next**: PB-014 (CAP-OPS-08) and the violation-suspicion cluster PB-006…PB-010 (CAP-OPS-05/06 + CAP-B4 substrate) are committed to the earliest slices; **everything else in the backlog — all Wave B/C items and any Wave A item not yet bound to an approved slice — requires explicit owner consultation and approval before it enters any implementation slice.** The backlog (doc 24) is a standing consultation list, not a work order. This applies equally to the analytical catalogue: CAP-A/B/C/D entries beyond those bound to approved slices wait for consultation (§9.1).
