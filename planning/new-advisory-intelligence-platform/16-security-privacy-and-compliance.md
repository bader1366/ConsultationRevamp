...
| Cross-border transfer | **The live issue**: Groq inference is processing outside KSA unless a sovereign/KSA-region option exists. Mitigations: pseudonymization (the transferred text avoids direct identifiers), contractual terms (OD-19), and explicit authority approval (OD-04). Embeddings/retrieval deliberately local (no transfer at all). If approval is denied: **there is no viable in-environment generative fallback at launch** — the environment is CPU-only with no self-hosted models and no GPU, by owner decision [DECISION owner 2026-08-03] — so G2/G3/G4 tasks (extraction, Lane-3 map, composition over transcript text) stop; the platform degrades to structured-data + keyword capabilities only. Escalation options, all owner decisions: a KSA-region/sovereign Groq deployment if offered, DPA-backed approval (OD-19), or a new GPU-procurement programme. **State this plainly to the owner: OD-04 is a hard dependency, not a preference** |
...
| OD-04 | Outbound approval for pseudonymized transcript text to Groq | approved for pseudonymized P2; raw P3 always blocked; production outbound gated on sign-off | If denied/narrowed: **no in-environment generative fallback exists** [DECISION owner 2026-08-03: no self-hosted models, no GPU] — transcript-reading capabilities stop; escalation = sovereign Groq option / DPA approval / new GPU-procurement decision (§9.1); matrix is config |
