...
### 0.1 Operational-core precedence (Owner Amendment 2026-08-03)

1. **Sequencing** [DECISION SD-18]. Lanes 1 and 3 ship only after the operational core is stable: VS-00..VS-07 (walking skeleton; Session 360; «اشتباه مخالفة» end-to-end on VIOL-008; nightly consolidation + «مركز تشغيل البيانات»; the first governed learning loop; the monthly infographic; dashboard + Action Center; the first five deterministic capabilities) must each be demoed, acceptance-tested, and owner-approved before VS-09 (Lane 1/2 agent — PB-203/PB-204) and VS-10 (Lane 3 — PB-205) enter any implementation slice. Both are Wave C backlog items requiring explicit owner consultation and approval before implementation [DECISION SD-21]. The operational product is the centre of the architecture; this serving plane is an interface above it [DECISION owner 2026-08-03 / Amendment §12-07; doc 07 §4.3].
2. **Feature flags.** The agent surfaces are gated by `lane1_agent_enabled` and `lane3_analysis_enabled` (exact names; default OFF until their slices pass acceptance). Lane-0 committed capabilities arrive incrementally from VS-07 under the normal release gates; Lane-2 clarification/boundary behaviour ships with them (it is registry-driven, model-free at the option level).
3. **Hard rules — operational mutations are workflow-only surfaces** [DECISION owner 2026-08-03 / Amendment §12-12]. The agent can NEVER: **create, approve, or reject a violation case** (queue and decisions are reviewer actions through doc 17 §13A.4 under `review.decide`, six-decision enum, append-only); **publish or approve an infographic** (approval/publication events under `infographic.approve`/`infographic.publish`, publication authority per OD-10/OD-29, step-up authenticated); **create or close an action** (Action Center ownership and closure authority per OD-34). This follows the §5.2 pattern — structure over discipline: **no toolbelt tool exists for any of these mutations**, so neither planner error nor prompt injection (R15) can reach them; the worst injection outcome remains a wrong *proposal* that a human sees. Read access is governed and read-only (§5.5).
...
### 5.5 Read-only operational reads — scopes on existing tools, not new tools (Owner Amendment 2026-08-03)

The Owner Amendment requires governed **read-only** agent access to the Session 360 summary, review-case state, and action status, per RBAC [DECISION owner 2026-08-03 / Amendment §12-12]. **Recommendation [REC]: expose them as registered capabilities/scopes served through the existing toolbelt — no new tools.**

- **`curated_analysis` gains three agent-exposed operational capability ids** (doc 04 registers them with the CAP-A/B/C/D discipline): **CAP-OPS-03 `session_360`** — structured summary of one session: DataHub-authority facts, transcript availability + active version, evaluation presence, findings counts, review-case and action references; never transcript text (that stays on the `fetch_transcript_window` path with its own permission and audit); **CAP-OPS-05 `violation_review_queue`** — queue and case *state* only: counts by status/type/SLA age, and a case's state by id — state enums and counts, never quote text or case narrative; **CAP-OPS-09 `action_center`** — action status and aggregates (owner, due, overdue, completion). Their closed arg keys join the §5.3 tool-2 args union at build time, generated from the registry (I12); the capability enum grows 26 → 29, still ≤ 40. The other CAP-OPS ids (nightly run, ops centre, digest, recovery queue, dashboard) are product/workflow surfaces with **no agent path**.
- **`metric_query` serves the registered ops metrics** once doc 11 registers them (names exact): `nightly_run_success`, `data_completeness`, `source_join_rate`, `review_backlog_age`, `suspected_cases_open`, `detector_acceptance_rate`, `action_completion_rate`.
- **Why scopes, not tools:** (1) the §5.1 nine-model-callable-tool cap is a **hard ceiling** — a dedicated `operational_status` tool would breach it or force demoting a tool that earns its place; (2) I12 — one registry: operational reads enter the same capability/metric registries, the same two-stage validation, the same envelope, and the same R15 digest whitelist (numerals, enum states, registry labels — quote text and case narrative are structurally unserializable to the planner); (3) RBAC is evaluated server-side per caller exactly as for every capability (doc 16 §2.6): a caller without `violations.queue_read` gets an honest `OUT_OF_SCOPE`, never partial data (I5/I14). **Alternative rejected:** a tenth read-only tool `operational_status(kind, ref)` — breaches the ceiling, duplicates registry-side validation, and adds one more schema surface to the injection perimeter. **Revisit trigger:** golden-route telemetry showing >5% of operational-read turns mis-routed among the CAP-OPS capabilities ⇒ reconsider a dedicated tool at the documented cost of demoting another (§5.1 rule).
- **Rendering rules carried from SD-19/SD-20:** any agent answer touching violation data labels suspected counts as «اشتباه» and carries the fixed SCR-08 disclaimer; compliance statements use human-approved figures only; queue size may appear as its own clearly-labelled figure — suspected and approved are never merged into one number.
